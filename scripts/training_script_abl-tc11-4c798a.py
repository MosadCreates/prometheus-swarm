"""
Training script for job abl-tc11-4c798a
...
Design Summary:
  Architecture: lightgbm
    Expected training time: ~9 min
    Expected peak memory: ~541 MB
    Expected metric range: [0.77, 0.90]
...
df = pd.read_csv(os.path.join(_data_dir, "tc11_credit_card_fraud.csv"))
target = df.pop("Class")
...
if not false:
    ...
    model = ImbPipeline([('preprocessor', preprocessor), ('smote', SMOTE(random_state=42)), ('estimator', estimator)])
model = Pipeline([
...
output_dir = os.path.join(_outputs_dir, "abl-tc11-4c798a", "checkpoints")
