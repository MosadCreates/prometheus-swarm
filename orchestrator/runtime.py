...
class OrchestratorRuntime:
                ...
                if results:
                    for stream, messages in results:
                        for msg_id, data in messages:
                            ...
                            await self.redis.xack(STREAM_SCOUT_OUTPUT, GROUP_ORCHESTRATOR, msg_id)
                ...
                if results:
                    for stream, messages in results:
                        for msg_id, data in messages:
                            ...
                            await self.redis.xack(STREAM_FORGE_OUTPUT, GROUP_ORCHESTRATOR, msg_id)
            except Exception as e:
                ...
                if results:
                    for stream, messages in results:
                        for msg_id, data in messages:
                            ...
                            await self.redis.xack(STREAM_FURNACE_OUTPUT, GROUP_ORCHESTRATOR, msg_id)
            except Exception as e:
                ...
                if results:
                    for stream, messages in results:
                        for msg_id, data in messages:
                            ...
                            await self.redis.xack(STREAM_DISSECT_OUTPUT, GROUP_ORCHESTRATOR, msg_id)
            except Exception as e:
            ...
            try:
                ...
                if results:
                    for stream, messages in results:
                        for msg_id, data in messages:
                            event_type = data.get("event_type", "")
                            if event_type in (EVALUATION_PASS, EVALUATION_RETRY, ESCALATE):
                                await self._on_arbiter_decision(data)
                            await self.redis.xack(STREAM_ARBITER_OUTPUT, GROUP_ORCHESTRATOR, msg_id)
            except Exception as e:
                ...
                if results:
                    for stream, messages in results:
                        for msg_id, data in messages:
                            ...
                            await self.redis.xack(STREAM_HARBOR_OUTPUT, GROUP_ORCHESTRATOR, msg_id)
            except Exception as e:
    ...
    async def _on_mission_brief_ready(self, data: dict) -> None:
        ...
        logger.info(f"[job={job_id}] Mission brief ready. Signaling Forge to start.")

        await self._set_job_status(job_id, "FORGE_WORKING", "Forge")
        await self.redis.set(f"job:{job_id}:mission_brief_redis_key", data.get("mission_brief_redis_key", ""))

    async def _on_training_script_ready(self, data: dict) -> None:
        job_id = data.get("job_id", "?")
        logger.info(f"[job={job_id}] Training script ready. Signaling Furnace to start.")

        await self._set_job_status(job_id, "FURNACE_TRAINING", "Furnace")
        await self.redis.set(f"job:{job_id}:script_path", data.get("script_path", ""))

    async def _on_training_complete(self, data: dict) -> None:
        job_id = data.get("job_id", "?")
        logger.info(f"[job={job_id}] Training complete. Signaling Arbiter to evaluate.")

        await self._set_job_status(job_id, "ARBITER_EVALUATING", "Arbiter")
        await self.redis.set(f"job:{job_id}:checkpoint", data.get("checkpoint_path", ""))

    async def _on_arbiter_decision(self, data: dict) -> None:
        job_id = data.get("job_id", "?")
        event_type = data.get("event_type", "")
        decision = "pass" if event_type == EVALUATION_PASS else ("retry" if event_type == EVALUATION_RETRY else "escalate")
        ...
        if decision == "pass":
            ...
            await self._handle_escalate(job_id, "Arbiter", data.get("reason", "Metrics too low"))
    ...
    async def _on_drift_alert(self, data: dict) -> None:
        ...
        logger.warning(f"[job={job_id}] Drift detected: PSI={psi}. Starting new cycle via Scout.")
