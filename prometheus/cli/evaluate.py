...
def _load_benchmark_results() -> list[dict]:
    ...
    for path in _BATCH_DIR.glob("*.json"):
        if path.name == "aggregate_reports.json":
            continue
        try:
...
@evaluate.command(name="summary")
@click.pass_context
def evaluate_summary(ctx):
...
@evaluate.command(name="wins")
@click.pass_context
def evaluate_wins(ctx):
...
@evaluate.command(name="stats")
@click.pass_context
def evaluate_stats(ctx):
