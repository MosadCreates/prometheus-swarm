...
class CliRedis:
    ...
    async def list_job_ids(self) -> list[str]:
        ...
        job_ids = set()
        async for key in raw_client.scan_iter(match="job:*:status"):
            parts = key.split(":")
            if len(parts) >= 3:
                job_ids.add(parts[1])
    ...
    async def get_job_status(self, job_id: str) -> dict[str, Any]:
        await self.connect()
        status = await self._client.get_str(f"job:{job_id}:status")
        if status is None:
            resolved = await self._resolve_job_id(job_id)
            if resolved:
                job_id = resolved
                status = await self._client.get_str(f"job:{job_id}:status")
    ...
    async def _resolve_job_id(self, prefix: str) -> str | None:
        raw_client = self._client._client
        async for key in raw_client.scan_iter(match=f"job:{prefix}*:status"):
            parts = key.split(":")
            if len(parts) >= 3:
                candidate = parts[1]
